...
**CRITICAL:** Before analyzing severity, context, or user impact, perform a complete pattern scan to identify ALL silent failure instances:

### Step 1: Locate All Error Handling Code
Scan the entire code block for:
- Try-catch blocks and their catch body contents
- `.catch()` chains on promises
- Conditional guards like `if (!data) return;`
- Fallback logic and default value assignments
- Operations within catch blocks
- Promise.allSettled() usage and result processing

### Step 2: Check Each Location Against This Checklist
For **every** try-catch, `.catch()`, or conditional guard found, systematically verify:

1. **Empty Catch Pattern**
   - Is the catch block empty? `catch { }` or `catch(e) { }`?
   - Is the catch body just `return;` with no logging?
   - Is the catch body just `return null;` or `return undefined;` with no logging?
   - Record: Location, what error types could occur here, why this is critical

2. **.catch(() => {}) Pattern**
   - Does code use `.catch(() => {})` or `.catch(e => {})` with empty handler?
   - Does code use `.catch(_ => {})` that discards the error?
   - Does code use `.catch(() => undefined)` or similar swallowing?
   - Record: Location, what promise rejection is being swallowed, what operations depend on this

3. **Count-Only Logging Pattern**
   - Does the catch block only do `failed++` or `count++` without capturing details?
   - Does code examine `Promise.allSettled()` results for `.length` or filtering counts only?
   - Does code log aggregate stats like "Synced 50/100" without logging which 50 failed?
   - Record: Location, what details are discarded, what items/operations failed

4. **Silent Early Return Pattern**
   - Does code have `if (!data) return;` without checking if this was an error or a legitimate null?
   - Does code return early on a falsy value without distinguishing error from missing-but-okay?
   - Does code use `?.` optional chaining to silently skip potentially critical operations?
   - Record: Location, whether this masks an actual error vs. legitimate absence

5. **Unhandled Operations in Catch Pattern**
   - Does the catch block call async functions that could throw? (e.g., `db.update()` in catch)
   - Does the catch block call I/O operations without their own try-catch?
   - Does the catch block execute logic that could fail independently?
   - Record: Location, what operations in catch could fail, consequences of those failures

6. **Error Swallowing with Fallback Pattern**
   ...
   - Does code silently use alternative behavior instead of error?
   - Record: Location, what fallback is used, how caller would know the primary path failed

7. **Insufficient Logging Pattern**
   ...
   - Does the catch block log a generic message without error details?
   - Does the catch block log but not include relevant context (IDs, operation name, state)?
   - Record: Location, what context is missing, what error details are lost

### Step 3: Document ALL Findings
List every pattern instance found with:
- **Exact location** (function name, line number range, code snippet)
- **Pattern type** (which of the above patterns applies)
- **What's being hidden** (which error types, what information is discarded)
- **Consequence** (what the caller/user cannot know happened)

Do not proceed to severity assessment until you have documented every single instance.
